...
public class OverwriteWindowViewModel : ReactiveObject
{
    ...
    {
        LoadData();
        OpenClientFolderCommand = new AsyncRelayCommand(async () =>
            ClientPath = await FileHelper.OpenFolderAsync(ClientPath, WindowHelper.OverwriteWindow));
        OpenServerFolderCommand = new AsyncRelayCommand(async () =>
            ServerPath = await FileHelper.OpenFolderAsync(ServerPath, WindowHelper.OverwriteWindow));
    }
