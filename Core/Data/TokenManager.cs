...
namespace PackForge.Core.Data
{
    ...
    public class TokenStore
    {
        ...
        public IReadOnlyList<Token> GetAllTokens() => _tokens.AsReadOnly();
    ...
    internal static class TokenManager
    {
        ...
        public static bool IsTokenStored(TokenType type)
        {
            ...
            bool onDisk = Validator.FileExists(GetTokenFilePath(type), LogEventLevel.Debug)
                          && new FileInfo(GetTokenFilePath(type)).Length > 0;
        ...
        public static async Task<string> RetrieveTokenValueByTypeAsync(TokenType tokenType)
        {
            try
            {
                ...
                byte[] encryptedTokenBytes = !string.IsNullOrWhiteSpace(memoryToken)
                    ? Convert.FromBase64String(memoryToken)
                    : diskTokenBytes;
