...
namespace PackForge.Core.Builders
{
    ...
    public static class ChangelogGenerator
    {
            ...
            Task<Dictionary<string, string>> oldTask =
                Task.Run(
                    () => Directory.EnumerateFiles(oldExport, "*.json", SearchOption.AllDirectories)
                        .Where(f => Path.GetRelativePath(oldExport, f).Contains(Path.DirectorySeparatorChar))
                        .ToDictionary(f => Path.GetRelativePath(oldExport, f), f => f), ct);

            Task<Dictionary<string, string>> newTask =
                Task.Run(
                    () => Directory.EnumerateFiles(newExport, "*.json", SearchOption.AllDirectories)
                        .Where(f => Path.GetRelativePath(newExport, f).Contains(Path.DirectorySeparatorChar))
                        .ToDictionary(f => Path.GetRelativePath(newExport, f), f => f), ct);
        ...
        internal static async Task<List<FileEntry>> GetFolderDiffAsync(
            string oldExport, 
            string newExport, 
            ...
            CancellationToken ct = default)
        {
            ...
            {
                await processBlock.SendAsync(key, ct);
            }
        ...
        internal static bool JTokenEquals(JToken t1, JToken t2)
        {
            ...
            return t1 switch
            {
                JObject o1 when t2 is JObject o2 =>
                    o1.Count == o2.Count &&
                    o1.Properties().All(p => o2.TryGetValue(p.Name, out JToken? v) && JTokenEquals(p.Value, v)),
                JArray a1 when t2 is JArray a2 =>
                    a1.Count == a2.Count &&
                    a1.All(item => a2.Any(i => JTokenEquals(item, i))),
