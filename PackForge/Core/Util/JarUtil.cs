...
namespace PackForge.Core.Util
{
    ...
    public record ModInfo(string FilePath, ModMetadata Metadata, int ClassFileCount, bool IsMcreator, bool OnlyJarInJars, List<ModInfo> NestedJars)
    {
        ...
        public static ModInfo NoData(string path) => new(
            path, 
            ModMetadata.Empty, 
            0, 
            false, 
        ...
        {
            return $"{Path.GetFileName(FilePath)} '{Metadata.DisplayName} - {Metadata.Version}' ({Metadata.ModId}) by {string.Join(", ", Metadata.Authors)} [{ClassFileCount} classes] [Mcreator: {IsMcreator}] [JarInJars: {NestedJars.Count}]";
        }
    }
    ...
    public static class JarUtil
    {
        ...
        {
            List<ZipArchiveEntry> nestedJarEntries = archive.Entries
                .Where(entry => entry.FullName.StartsWith("META-INF/jarjar", StringComparison.OrdinalIgnoreCase) &&
        ...
        {
            IEnumerable<ZipArchiveEntry> validEntries = archive.Entries
                .Where(entry => !entry.FullName.StartsWith("assets", StringComparison.OrdinalIgnoreCase) &&
